  ...
  <div class="card-content">
    ...
    <form method="post" action="{{route('stocktaking.update',$stocktaking->id)}}">
      ...
      <div class="field">
        <div class="control">
          <input class="input is-primary" type="text" name="quantity" placeholder="Cantidad"
            value="{{$stocktaking->quantity}}">
        </div>
      </div>

      <div class="field">
        <div class="control">
          <input class="input is-primary" type="text" name="description" placeholder="Descripcion"
      ...
      <select name="product_id" multiple size="3">
        ...
        <option value=" {{$product['id'] }}" @if($product->id === $stocktaking->product_id )
          selected
          @endif
          >{{$product['name']}}</option>
        @endforeach
